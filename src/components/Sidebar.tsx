      ...
      <aside className={cn(
        ...
        <div className={cn(
          ...
          <div className="relative shrink-0">
            ...
            {/* AI Chicken Badge */}
            <div className="absolute -top-1 -left-1 w-5 h-5 bg-amber-400 rounded-full flex items-center justify-center shadow-sm border border-white dark:border-slate-800">
        ...
        <nav className="flex-1 p-4 space-y-2">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon size={20} className={cn("relative z-10", isActive ? theme.accent : "text-slate-400 group-hover:text-white")} />
                {!isCollapsed && (
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              onClick={() => setActiveTab(item.id)}
              className={cn(
                "flex flex-col items-center py-1 px-1 rounded-xl min-w-[64px] transition-all duration-200 relative",
                isActive ? theme.text : "text-slate-400 hover:text-slate-600 dark:hover:text-slate-300"
              )}
            >
              <div className={cn(
                "p-1.5 rounded-full mb-0.5 transition-all duration-200 relative", 
