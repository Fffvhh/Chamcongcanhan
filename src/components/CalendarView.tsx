...
import { ChevronLeft, ChevronRight, CheckCircle2, XCircle, Clock, Calendar as CalendarIcon, X, Moon, Wallet, Plus, Trash2, List } from 'lucide-react';
...
export function CalendarView() {
  ...
  const [activeSubTab, setActiveSubTab] = useState<'calendar' | 'expenses' | 'savings'>('calendar');
  const { records, updateRecord, getMonthlySummary, fetchMonthData, salarySettings, theme, savingsGoal, updateSavingsGoal } = useAttendance();
  ...
  // Savings State
  const [localSavingsGoal, setLocalSavingsGoal] = useState(savingsGoal?.targetAmount?.toString() || '');
  const [isSavingGoal, setIsSavingGoal] = useState(false);

  useEffect(() => {
    setLocalSavingsGoal(savingsGoal?.targetAmount?.toString() || '');
  }, [savingsGoal?.targetAmount]);

  const handleSaveSavingsGoal = async () => {
    setIsSavingGoal(true);
    try {
      const val = parseInt(localSavingsGoal.replace(/\D/g, '')) || 0;
    ...
    <div className="p-4 md:p-8 max-w-6xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6 md:mb-8 gap-4">
          <div>
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900 tracking-tight">
              {activeSubTab === 'calendar' ? 'Lịch chấm công' : activeSubTab === 'expenses' ? 'Kế hoạch chi tiêu' : 'Mục tiêu tiết kiệm'}
            </h2>
            <p className="text-sm md:text-base text-slate-500 mt-1">
              {activeSubTab === 'calendar' ? 'Theo dõi quá trình làm việc của bạn' : activeSubTab === 'expenses' ? 'Quản lý tài chính cá nhân hàng tháng' : 'Thiết lập và theo dõi mục tiêu tiết kiệm'}
            </p>
          </div>
          <div className="flex flex-col md:flex-row items-center gap-3 w-full md:w-auto">
            <div className="flex bg-slate-100 p-1 rounded-xl w-full md:w-auto overflow-x-auto">
              <button 
                ...
                className={cn("flex-1 md:flex-none px-4 py-2 text-sm font-medium rounded-lg transition-all flex items-center justify-center gap-2 whitespace-nowrap", activeSubTab === 'calendar' ? cn("bg-white shadow-sm", theme.text) : "text-slate-500 hover:text-slate-700")}
              >
              ...
              <button 
                ...
                className={cn("flex-1 md:flex-none px-4 py-2 text-sm font-medium rounded-lg transition-all flex items-center justify-center gap-2 whitespace-nowrap", activeSubTab === 'expenses' ? cn("bg-white shadow-sm", theme.text) : "text-slate-500 hover:text-slate-700")}
              >
                <Wallet size={16} />
                <span>Chi tiêu</span>
              </button>
              <button 
                ...
                className={cn("flex-1 md:flex-none px-4 py-2 text-sm font-medium rounded-lg transition-all flex items-center justify-center gap-2 whitespace-nowrap", activeSubTab === 'savings' ? cn("bg-white shadow-sm", theme.text) : "text-slate-500 hover:text-slate-700")}
              >
                <Wallet size={16} />
                <span>Tiết kiệm</span>
              </button>
      ...
      {activeSubTab === 'savings' && (
        <div className="animate-in fade-in duration-300">
          <div className={cn("bg-white rounded-2xl p-6 shadow-sm border max-w-xl mx-auto", theme.borderLight)}>
            <div className="flex items-center gap-4 mb-6">
              <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center shrink-0", theme.bgLight, theme.accent)}>
                <Wallet size={24} />
              </div>
              <div>
                <h3 className="text-lg font-bold text-slate-800">Mục tiêu tiết kiệm</h3>
                <p className="text-sm text-slate-500">Thiết lập số tiền bạn muốn đạt được</p>
              </div>
            </div>
            <div className="flex flex-col sm:flex-row items-center gap-3">
              <div className="relative flex-1 w-full">
                <input 
                  type="text"
                  value={localSavingsGoal ? parseInt(localSavingsGoal.replace(/\D/g, '') || '0').toLocaleString('vi-VN') : ''}
                  onChange={(e) => setLocalSavingsGoal(e.target.value)}
                  className={cn("w-full bg-slate-50 border text-lg font-black text-slate-800 rounded-xl px-4 py-3 pr-12 focus:outline-none focus:ring-2 transition-all", theme.borderLight, theme.focus.replace('ring-', 'border-'))}
                  placeholder="0"
                />
              ...
              <button
                ...
                className={cn("w-full sm:w-auto px-6 py-3 text-white rounded-xl font-bold transition-all shadow-md active:scale-95 whitespace-nowrap", theme.primary, theme.hover)}
              >
