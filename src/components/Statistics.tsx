...
import { format, parseISO, startOfMonth, endOfMonth, eachDayOfInterval, isWeekend, addMonths, subMonths, addYears, subYears, differenceInMinutes, subDays } from 'date-fns';
...
export function Statistics() {
  ...
  const stats = useMemo(() => {
    ...
    const weeklyMoodCounts = { '😭': 0, '😥': 0, '🔥': 0, '🥰': 0 };

    const chartData = daysInMonth.map(day => {
      const dateStr = format(day, 'yyyy-MM-dd');
      const record = records[dateStr];
      
      if (record?.mood && moodCounts.hasOwnProperty(record.mood)) {
        moodCounts[record.mood as keyof typeof moodCounts]++;
      }
    ...
    last7Days.forEach(day => {
      const dateStr = format(day, 'yyyy-MM-dd');
      const record = records[dateStr];
      if (record?.mood && weeklyMoodCounts.hasOwnProperty(record.mood)) {
        weeklyMoodCounts[record.mood as keyof typeof weeklyMoodCounts]++;
      }
    });

    const last7DaysMoods = last7Days.map(day => {
      const dateStr = format(day, 'yyyy-MM-dd');
      const record = records[dateStr];
      return {
        date: day,
  ...
  const handleProfessionalExportPDF = () => {
    ...
    const monthYear = isMonth ? format(currentMonth, 'MM/yyyy') : format(currentMonth, 'yyyy');
    const data = isMonth 
      ? stats.chartData.map(d => d.record).filter(Boolean) 
      : yearlyStats?.chartData || [];
    const summary = isMonth ? stats : yearlyStats;
    exportToProfessionalPDF(data, salarySettings, monthYear, summary);
  };
  ...
  const handleProfessionalExportExcel = () => {
    ...
    const data = isMonth 
      ? stats.chartData.map(d => d.record).filter(Boolean) 
      : yearlyStats?.chartData || [];
    exportToExcel(data, salarySettings, monthYear);
  };
            ...
            <div className="bg-white p-4 md:p-6 rounded-2xl md:rounded-3xl shadow-sm border border-slate-100 print-hide">
              <div className="flex items-center justify-between mb-4 md:mb-6">
                <div className="flex items-center gap-2">
                  <Activity size={20} className={theme.accent} />
                  <h3 className="text-lg md:text-xl font-semibold text-slate-800">Thống kê cảm xúc</h3>
                </div>
                <div className="flex items-center gap-1.5 px-3 py-1 bg-slate-50 rounded-full border border-slate-100">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Trực quan</span>
                </div>
              </div>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">7 ngày gần nhất</p>
                    <span className="text-[10px] font-medium text-slate-400 italic">Cập nhật hàng ngày</span>
                  </div>
                  <div className="flex items-center justify-between gap-2 overflow-x-auto pb-2 custom-scrollbar">
                    {stats.last7DaysMoods.map((day, idx) => (
                      <div key={idx} className="flex flex-col items-center gap-2 min-w-[48px]">
                        <div className={cn(
                          "w-10 h-10 rounded-2xl flex items-center justify-center text-xl shadow-sm transition-all duration-300",
                          day.mood ? "bg-white border border-slate-100 scale-100" : "bg-slate-50 border border-dashed border-slate-200 scale-90 opacity-50"
                        )}>
                          {day.mood || "—"}
                        </div>
                        <span className={cn(
                          "text-[9px] font-bold uppercase tracking-tighter",
                          format(day.date, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd') ? theme.text : "text-slate-400"
                        )}>
                          {format(day.date, 'yyyy-MM-dd') === format(new Date(), 'yyyy-MM-dd') ? "Hôm nay" : format(day.date, 'dd/MM')}
                        </span>
                      </div>
                    ))}
                  </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      <div className={cn("w-1 h-4 rounded-full", theme.bg)}></div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Tổng hợp tuần</p>
                    </div>
                    <div className="grid grid-cols-4 gap-2">
                      {Object.entries(stats.weeklyMoodCounts).map(([emoji, count]) => (
                        <div key={emoji} className="group relative flex flex-col items-center p-3 bg-slate-50/50 hover:bg-white rounded-2xl border border-transparent hover:border-slate-100 hover:shadow-sm transition-all duration-300">
                          <span className="text-xl mb-1 group-hover:scale-110 transition-transform">{emoji}</span>
                          <span className="text-sm font-black text-slate-800">{count}</span>
                          {count > 0 && (
                            <div className={cn("absolute -top-1 -right-1 w-2 h-2 rounded-full", theme.bg)}></div>
                          )}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-2">
                      <div className={cn("w-1 h-4 rounded-full bg-indigo-500")}></div>
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Tổng hợp tháng {format(currentMonth, 'MM')}</p>
                    </div>
                    <div className="grid grid-cols-4 gap-2">
