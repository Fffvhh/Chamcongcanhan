...
interface AttendanceContextType {
  ...
  updateSavingsGoal: (target: number) => Promise<void>;
  addSavingsTransaction: (amount: number, note: string, type: 'deposit' | 'withdraw') => Promise<void>;
...
export function AttendanceProvider({ children }: { children: React.ReactNode }) {
  ...
  const getWorkingHours = useCallback((record: AttendanceRecord) => {
  ...
  const getMonthlySummary = useCallback((monthDate: Date) => {
    ...
    for (let day = 1; day <= totalDaysInMonth; day++) {
      ...
      if (record) {
        ...
        const h = getWorkingHours(record);
        totalHours += h.total;
  ...
  }, [records, salarySettings.baseWage, getWorkingHours]);
  ...
  useEffect(() => {
    ...
    Object.values(records).forEach(record => {
      if (record.date.startsWith(currentMonthStr)) {
        const h = getWorkingHours(record);
        total += h.total;
      }
    });
    setTotalHours(total);
  }, [user, records, isLoaded, getWorkingHours]);
  ...
  // Firestore Sync - Savings
  useEffect(() => {
    if (!user) return;
    const currentYear = new Date().getFullYear();
    const savingsDoc = doc(db, `users/${user.uid}/savings/${currentYear}`);
    
    const unsubscribe = onSnapshot(savingsDoc, (snapshot) => {
      if (snapshot.exists()) {
      ...
      handleFirestoreError(error, OperationType.GET, `users/${user.uid}/savings/${currentYear}`);
    });

    return () => unsubscribe();
  }, [user]);

  const updateSavingsGoal = useCallback(async (target: number) => {
    if (!user) return;
    const currentYear = new Date().getFullYear();
    try {
      await setDoc(doc(db, `users/${user.uid}/savings/${currentYear}`), {
        targetAmount: target,
        updatedAt: new Date().toISOString(),
        uid: user.uid
      }, { merge: true });
    } catch (error) {
      handleFirestoreError(error, OperationType.UPDATE, `users/${user.uid}/savings/${currentYear}`);
    }
  }, [user]);

  const addSavingsTransaction = useCallback(async (amount: number, note: string, type: 'deposit' | 'withdraw') => {
    if (!user) return;
    const currentYear = new Date().getFullYear();
    ...
    const newTransactions = [newTransaction, ...savingsGoal.transactions];
  ...
  const deleteSavingsTransaction = useCallback(async (id: string) => {
    if (!user) return;
    const currentYear = new Date().getFullYear();
    const transactionToDelete = savingsGoal.transactions.find(t => t.id === id);
    if (!transactionToDelete) return;

    const newTransactions = savingsGoal.transactions.filter(t => t.id !== id);
    const newCurrentAmount = transactionToDelete.type === 'deposit'
      ? savingsGoal.currentAmount - transactionToDelete.amount
      : savingsGoal.currentAmount + transactionToDelete.amount;
  ...
  }), [user, records, userRole, isLoaded, darkMode, salarySettings, deviceHistory, totalHours, history, hasMore, isLoadingHistory, loadMoreHistory, fetchMonthData, fetchYearData, themeColor, setThemeColor, theme, journeys, fetchJourneys, savingsGoal, updateSavingsGoal, addSavingsTransaction, deleteSavingsTransaction]);
