...
import { ImagePlus, Trash2, X, Calendar, Camera, ChevronLeft, ChevronRight, SwitchCamera, Clock } from 'lucide-react';
...
export const Memories = () => {
  ...
  const startCamera = async (mode = facingMode) => {
    ...
    try {
      ...
      // Request high resolution (4K ideal, fallback to HD)
      const constraints = {
        video: {
          facingMode: mode,
          width: { ideal: 3840 },
      ...
      if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
        alert("Quyền truy cập camera bị từ chối. Vui lòng:\n1. Bấm vào biểu tượng ổ khóa trên thanh địa chỉ.\n2. Cho phép (Allow) Camera.\n3. Tải lại trang.");
        setIsCameraOpen(false);
        return;
      }
      ...
      try {
        ...
        if (fallbackErr.name === 'NotAllowedError' || fallbackErr.name === 'PermissionDeniedError') {
          alert("Quyền truy cập camera bị từ chối. Vui lòng kiểm tra cài đặt trình duyệt.");
        } else {
  ...
  const capturePhoto = async () => {
    if (videoRef.current && canvasRef.current) {
      ...
      // Draw background pill with blur effect (simulated with shadow)
      ...
      // Draw text with subtle shadow for extreme clarity
      ...
      // High quality JPEG output (0.92 is excellent balance)
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      img.onload = () => {
        ...
        // Increase max dimensions for HD/4K quality
  ...
  const handleSave = () => {
    ...
    addMemory(memoryData)
      .catch((error) => {
        ...
        alert("Có lỗi xảy ra khi lưu kỉ niệm. Vui lòng thử lại.");
      })
      .finally(() => {
        setIsSaving(false);
      });
                ...
                return (
                  <div key={memory.id} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
                    ...
                    <div className="mt-4 flex justify-end items-center">
                      <button 
                        onClick={() => {
                          if (window.confirm('Bạn có chắc chắn muốn xóa kỉ niệm này?')) {
                            deleteMemory(memory.id);
                            if (viewingMemories.length === 1) {
                              setViewingMemories(null);
                              setExpandedImageId(null);
                            } else {
                              setViewingMemories(viewingMemories.filter(m => m.id !== memory.id));
                            }
                          }
                        }}
