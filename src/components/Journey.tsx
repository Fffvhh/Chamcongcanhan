...
import { MapPin, Navigation, Plus, Map as MapIcon, Calendar, Trash2, X, Loader2, Compass, Share2, Edit2, Copy, Clock, RefreshCw, ChevronDown, ChevronUp } from 'lucide-react';
...
export const Journey = () => {
  ...
  const handleEdit = (journey: JourneyType) => {
    ...
    setIsNoteExpanded(!!journey.note);
            ...
            <div className="p-4 space-y-4">
              <div className="space-y-3">
                ...
                <div>
                  ...
                  >
                    <label className="text-[9px] uppercase tracking-widest font-black text-slate-400 mb-1 block ml-1 cursor-pointer group-hover:text-slate-600 dark:group-hover:text-slate-300 transition-colors">
                      Ghi chú {note && <span className="text-emerald-500">*</span>}
                    </label>
                    <button 
                      type="button"
                  ...
                  {isNoteExpanded && (
                    <div className="relative mt-1 animate-in fade-in slide-in-from-top-2 duration-200">
                      <textarea 
                        ref={textareaRef}
                        value={note}
                        onChange={(e) => setNote(e.target.value)}
                        rows={2}
                        className={cn("w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-white rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 transition-all font-medium resize-none placeholder:text-slate-400 overflow-hidden min-h-[80px]", theme.ring)}
            ...
            <div key={journey.id} className="group bg-white dark:bg-slate-900 rounded-[1.5rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-md transition-all duration-300 relative flex flex-col overflow-hidden">
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                <div className="space-y-2.5 mb-3">
                  ...
                  {journey.note && (
                    <p className="text-[10px] text-slate-500 dark:text-slate-400 italic whitespace-pre-wrap leading-relaxed bg-slate-50 dark:bg-slate-800/30 p-2 rounded-lg border border-slate-100 dark:border-slate-800/50">
                      "{journey.note}"
                    </p>
                  )}
                </div>
